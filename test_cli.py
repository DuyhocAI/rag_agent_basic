...
def _print_warn(t: str)   -> None: _ui_print_warn(t)
...
def _chatbot_repl(base: str, session_id: str = "default") -> str:
    ...
    while True:
        ...
        def _tick_progress():
            """Poll /chat/activity and show live agent status while waiting."""
            last_count = 0
            last_line  = ""
            poll_every = 3   # seconds between activity polls
            tick       = 0
            while not _stop_tick.is_set():
                elapsed_s = int(time.time() - _t0)
                mins, secs = divmod(elapsed_s, 60)

                # Every poll_every seconds, fetch latest activity from server
                if tick % poll_every == 0:
                    try:
                        act = _http_get(
                            f"{base}/chat/activity/{session_id}?since={last_count}",
                            timeout=3)
                        if act and act.get("entries"):
                            for entry in act["entries"]:
                                print(f"\r  {entry[:72]:<72}", flush=True)
                            last_line  = act["entries"][-1][:60]
                            last_count = act.get("total", last_count)
                    except Exception:
                        pass

                status_line = last_line if last_line else "thinking"
                dots = "." * (tick % 4 + 1)
                print(f"\r  [{mins:02d}:{secs:02d}] {status_line}{dots:<5}   ",
                      end="", flush=True)
                tick += 1
                _stop_tick.wait(1)
        ...
        _tick_thread = threading.Thread(target=_tick_progress, daemon=True)
        _tick_thread.start()
