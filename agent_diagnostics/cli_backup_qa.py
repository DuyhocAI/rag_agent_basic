...
"""
import fix_encoding  # Fix loi encoding tieng Viet
rag_agent/cli.py
...
CLI_LOG_PATH    = Path(r"C:\Bao_Duy\rag_agent\cli.log")
POLL_INTERVAL   = 2        
MAX_FIX_RETRIES = 5        
PROGRESS_WIDTH  = 40       
...
def _init_cli_log() -> None:
    # Xoa du lieu cu va bat dau file log moi cho phien chay hien tai
    CLI_LOG_PATH.parent.mkdir(parents=True, exist_ok=True)
    with open(CLI_LOG_PATH, "w", encoding="utf-8") as f:
        f.write(f"--- Bat dau phien CLI: {_now_iso()} ---\n")

def _log_cli(text: str) -> None:
    try:
        # Loai bo ma mau ANSI truoc khi luu vao file txt
...
def _print_info(text: str) -> None:
    msg = f"{C_CYAN}[{_now_str()}]{C_RESET} {text}"
    print(msg)
    _log_cli(msg)

def _print_ok(text: str) -> None:
    msg = f"{C_GREEN}[{_now_str()}] {text}{C_RESET}"
    print(msg)
    _log_cli(msg)

def _print_warn(text: str) -> None:
    msg = f"{C_YELLOW}[{_now_str()}]  {text}{C_RESET}"
    print(msg)
    _log_cli(msg)

def _print_err(text: str) -> None:
    msg = f"{C_RED}[{_now_str()}]  {text}{C_RESET}"
    print(msg)
    _log_cli(msg)
...
def _base_url(cfg: dict) -> str:
    ...
    if host in ("0.0.0.0", ""):
        host = "127.0.0.1"
    # Mac dinh dung port 8765 cho dong bo he thong
...
def cmd_run(base: str, task: str) -> None:
    ...
    if not resp:
        _print_err("Could not reach agent server. Start it with: python server.py")
        return
...
def cmd_status(base: str) -> None:
    ...
    stats = mem.get("stats", {})
    if info:
        msg = f"\n  {C_BOLD}Server{C_RESET}\n    Active : {C_YELLOW}{info.get('active', 0)}{C_RESET}\n    Done   : {C_GREEN}{info.get('done', 0)}{C_RESET}\n    Errors : {C_RED}{info.get('errors', 0)}{C_RESET}\n    Total  : {info.get('total', 0)}"
        print(msg)
        _log_cli(msg)
    else:
        _print_warn("Server not reachable.")
    msg_mem = f"\n  {C_BOLD}Memory  {MEMORY_PATH}{C_RESET}\n    Total  : {stats.get('total', 0)}\n    Passed : {C_GREEN}{stats.get('passed', 0)}{C_RESET}\n    Failed : {C_RED}{stats.get('failed', 0)}{C_RESET}"
    print(msg_mem)
    _log_cli(msg_mem)
...
def cmd_task(base: str, task_id: str) -> None:
    ...
    msg = [
        f"  Task ID : {C_YELLOW}{info.get('task_id','?')}{C_RESET}",
        f"  Status  : {sc}{info.get('status','?')}{C_RESET}"
    ]
    t = info.get("task", "")
    if t:
        msg.append(f"  Task    : {t[:100]}")
    r = info.get("result", {})
    if r:
        msg.append(f"  Score   : {C_GREEN}{r.get('score','N/A')}{C_RESET}")
        msg.append(f"  Passed  : {r.get('passed','N/A')}")
        msg.append(f"  Dir     : {r.get('project_dir','N/A')}")
    if info.get("error"):
        msg.append(f"  Error   : {C_RED}{info['error']}{C_RESET}")
...
def cmd_memory() -> None:
    ...
    mem   = _memory_load()
    stats = mem.get("stats", {})
    msg_mem = f"  Total tasks : {stats.get('total', 0)}\n  Passed      : {C_GREEN}{stats.get('passed', 0)}{C_RESET}\n  Failed      : {C_RED}{stats.get('failed', 0)}{C_RESET}"
    print(msg_mem)
    _log_cli(msg_mem)
    ...
    if tasks:
        ...
        for t in tasks[-10:]:
            ...
            task_msg = f"    {sc}[{ts}]{C_RESET} score={t.get('score','?')}  {t.get('task','')[:60]}"
            print(task_msg)
            _log_cli(task_msg)
    ...
    if errors:
        ...
        for e in errors[-5:]:
            ...
            err_msg = f"    {C_RED}[{ts}]{C_RESET} {e.get('error','')[:80]}"
            print(err_msg)
            _log_cli(err_msg)
...
def _repl(base: str) -> None:
    ...
    mem   = _memory_load()
    stats = mem.get("stats", {})
    intro_msg = f"  Memory: {MEMORY_PATH}  |  Tasks recorded: {stats.get('total', 0)}"
    print(intro_msg)
    _log_cli(intro_msg)
    print(_REPL_HELP)
    _log_cli("Displayed help menu.")
    ...
    while True:
        ...
        if cmd in ("exit", "quit", "q"):
            ...
            # Chap nhan lenh la mot prompt truc tiep va thuc thi luon
            _print_info("Truyen input thanh task va bat dau xu ly...")
            cmd_run(base, raw)
...
def main() -> None:
    # Khoi tao xoa sach log moi khi bat dau phien
    _init_cli_log()
